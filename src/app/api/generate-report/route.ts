...
function generateFallbackReport(
  ...
  return {
    ...
    biggestExposure: unknownSpots.some((bs) => bs.id === "health-insurance")
      ? `An uninsured hospital stay in ${parentCity} costs ₹2-5 lakh. Confirm your parents' insurance status this week.`
      : unknownSpots.some((bs) => bs.id === "will")
        ? `No will means 5-10 years in court for property disputes. A registered will is the single highest-impact step.`
        : `${unknownSpots.length} financial blind spots mean assets could go untracked. One conversation can close most of these gaps.`,
