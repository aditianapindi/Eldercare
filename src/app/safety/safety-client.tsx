...
/**
 * Hero CTA row for /safety — branches on auth state.
 * Anonymous: primary "Start your free vault" → assessment funnel + anchor to scams
 * Authenticated: primary "Back to my vault" → /vault + anchor to scams
 */
export function SafetyHeroCTAs() {
  const { user } = useAuth();

  // Default to the anonymous CTA during loading so SSR renders real content.
  // Signed-in users see anonymous CTAs briefly until auth resolves, then
  // switch to "Back to my vault →".
  if (user) {
    return (
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <a
          href="#scams"
          className="text-ink-tertiary text-base hover:text-ink transition-colors"
        >
          ↓ Read the scam guide
        </a>
      </div>
...
/**
 * Sticky bottom bar for /safety — branches on auth state.
 * Anonymous: "Set up your family vault..." → assessment funnel
 * Authenticated: "Back to your family vault." → /vault directly
 */
export function SafetyStickyBar() {
  const { user } = useAuth();
  ...
  const href = user ? "/vault" : "/assess?source=safety";
