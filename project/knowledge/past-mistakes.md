...
## Inherited from an earlier session — `git init` ran in the wrong directory [RESOLVED SESSION 11]
**What:** The `.git` for this project lived at `/Users/aditianapindi/.git` (home folder) instead of `/Users/aditianapindi/Z/.git` (project folder). Intent was always a Z-level repo but `git init` was run from the parent by accident. Discovered in session 8 via `.gitignore` interference.
**Resolved:** Session 11 — see below. The full story was more tangled than this entry captured: a SECOND git repo at `~/Z/src/.git` was the real one connected to `github.com/aditianapindi/Eldercare`, and nobody noticed for 4 sessions.

## 2026-04-11 (session 11) — Nested `.git` repos hidden from `git rev-parse --show-toplevel`
**What:** Planned a full repo rewrite based on the assumption that `~/.git` (home-level, misplaced) was the only git repo for the project. It wasn't. A second `.git` at `~/Z/src/.git` existed simultaneously — connected to `github.com/aditianapindi/Eldercare` — and that was the actual source of record (26 granular commits through Apr 10). Sessions 7-10 code work (vault sharing, documents, upcoming feed, fraudguard, safety pages) was never committed into `Z/src/.git`, only into the misplaced home repo, which had no remote. Vercel CLI kept deploying from `Z/src/` directly, so production stayed current but GitHub rotted silently for 4 sessions. I nearly force-pushed the home-level rewrite over the real GitHub history, which would have wiped 26 real commits.
**Pattern:** `git rev-parse --show-toplevel` only reports the nearest enclosing repo going UP. It is blind to nested repos one level down. Before trusting ANY git-repair plan — especially destructive ones like `filter-repo` + `rm -rf .git` — run `find <project-root> -name .git -type d` (or check `find ~ -maxdepth 3 -name .git -type d` when operating in a home dir). Also: session journals are not authoritative about git state. They capture what the previous session's author saw, which may miss what's actually on disk. Always re-verify with fresh commands.
**Fix:** `rm -rf ~/Z/.git` (the dead-end rewritten repo) → commit Sessions 7-10 work into `~/Z/src/.git` as 2 logical commits → `git push origin main` → create `~/Z/src/project/` and move `docs/`, `knowledge/`, `db/`, `CLAUDE.md` into it → commit + push. Result: 29 commits on GitHub, nothing lost, real history preserved. Also added `feedback_git_before_vercel.md` to auto-memory as a tripwire for future direct-Vercel deploys without a git check first.
